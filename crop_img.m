            ...
            for m = 1:n
                ...
                if pad_x>pad_y
                    if rem(pad_x-pad_y,2)==0
                        if top_left(k,2)-(pad_x-pad_y)/2<0
                            I = imcrop(I,[top_left(k,1) 1 pad_x pad_x]);
                            [row,col,dim] = size(I);
                            for t = 1:t1
                                gt_new(t,1,m) = (gt(t,1,m)-top_left(k,1)+1)*256/row;
                                gt_new(t,2,m) = gt(t,2,m)*256/col;
                            end
                        else
                            ...
                            for t = 1:t1
                                gt_new(t,1,m) = (gt(t,1,m)-top_left(k,1)+1)*256/row;
                                gt_new(t,2,m) = (gt(t,2,m)-top_left(k,2)+fix((pad_x-pad_y)/2)+1)*256/col;
                            end
                        end
                    else 
                         if top_left(k,2)-(pad_x-pad_y)/2<0
